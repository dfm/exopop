...
For the remainder of this \paper, we will also model the occurence rate as a
histogram in logarithmic bins of period $\ln\period$ and radius $\ln\radius$.
...
The exact shape of the implied prior on planet radius depends on the assumed
priors on radius ratio and stellar radius but in the limit of a broad uniform
prior on each quantity, the implicit prior at small physical radius is
approximately uniform; this is the assumption that we make.
...
First, since the radius uncertainties are dominated by uncertainties in the
stellar parameters, it is not possible to use the published catalog
(\citealt{petigura}) to compute constraints on radius ratios.
\dfmtodo{finish this}
