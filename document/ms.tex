...
In \app{inv-det-eff}, we show that the inverse-detection-efficiency method can
...
in the \sect{hier}, we present the basic probabilistic question and derive a
...
Finally, for all the results in this \paper, we'll model the rate density as
a piecewise constant step function
...
In \app{inv-det-eff}, we use the model in \eq{rate-model} and neglect
observational uncertainties, we can derive the analytic maximum likelihood
solution for the step heights.
...
\dfmtodo{Do we want to add a comment about giant planets? Or maybe some sort
of cumulative distribution plot/code snippet?}

\dfmtodo{Add sentence listing posterior constraints on hyperparameters.}
...
This inconsistency is due to the different assumptions made the detailed cause
...
This analysis (labeled ``negligible uncertainties'' in \fig{comparison}) gives
the result
\begin{eqnarray}
...
Firstly, while \citet{dong} did use a somewhat flexible extrapolation
function than (a power law in log period), it is still very restrictive
considering the large range of periods; a broken power law might have been
more applicable.
...
based on the \citet{petigura} sample an underestimate of the \True\ rate.
...
\begin{figure}[p]
\begin{center}
\includegraphics[width=0.8\textwidth]{figures/smooth/rate.pdf}
\end{center}
\caption{%
{\bf Simulated data}.
...
\begin{figure}[p]
\begin{center}
\includegraphics[width=0.8\textwidth]{figures/simulation/rate.pdf}
\end{center}
\caption{%
{\bf Simulated data}.
...
\begin{figure}[p]
\begin{center}
\includegraphics[width=0.8\textwidth]{figures/results/rate.pdf}
\end{center}
\caption{%
...
The ``negligible uncertainties'' value is given by applying the model
