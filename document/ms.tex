...
Under strong assumptions, these data have been used to make an extrapolated
estimate of the probability or rate at which Sun-like stars host Earth
analogs.
We relax some of these assumptions, only requiring that the occurrence rate be
a smooth function of period and radius (using a Gaussian process) and find
that the rate of Earth analogs per star is about 0.02 (per natural logarithmic
...
\dfmtodo{From Tim: In a new paragraph here, I think we should say something
like ``In addition, there are two philosophically different ways of applying
this detection efficiency to derive planet occurrence rates.''
 Then first describe the inv-det-eff (cite Howard2012, Dong+Zhu2012,
Swift+2013, Dress+Charb2013), then the forward-modeling concept, advocated by
Youdin (2011) in a max-like framework.}
...
of interest (\dfmtodo{give a number and CITE}).
...
multiple transiting systems into account (\dfmtodo{CITE}) and this would be
possible using a conditional distributions (as developed by
\citealt{tremaine}).
...
\dfmtodo{We should explain why our number is a lower limit here.}
...
\mean\ and covariance matrix \cov\ that depends on the bin locations
...
It is also worth noting that we're using the \emph{median} transit probability
