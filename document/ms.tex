...
No true extra-Solar Earth analog is known.
...
In contrast to previous work (\dfmtodo{CITE?}), we don't define ``Earth
analog'' in terms of habitability or composition.
...
Makes our estimates overestimates.
Go to a mixture model.

\item {\bf Known observational uncertainties}\quad
Assumed Gaussian. Would be better with samples.

\item {\bf Given empirical detection efficiency}\quad
Single vs multi.
In detail there are limitations: radius vs ratio. Stellar params.

\item {\bf Smooth rate function}\quad
No reason for the function to be smooth.
Phase transitions.
Composition etc.
