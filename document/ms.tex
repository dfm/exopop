...
In the next Section, we introduce our method for incorporating the effects of
observational uncertainties into the derivation of occurrence rates.
...
in the next section, we present the basic probabilistic question and derive a
computationally tractable method for inferring the population of exoplanets.
In \sect{model}, we explain how to incorporate the effects of survey
completeness into our inferences and derive the key equation for our method:
\eq{money}.
...
\end{array}\right.
\end{eqnarray}
where the parameters $\ratepar_j$ are the log step heights and the bin edges
are fixed \foreign{a priori}.
...
We would also like to acknowledge the anonymous referee for suggestions that
