...
Earth analogs.
Complicating these studies are that every catalog is censored by non-trivial
selection effects and detection efficiencies, and every system property
measurement (period, radius, \etc) is noisy.
Here we present a general hierarchical probabilistic framework for making
justified inferences about the occurrence rate of exoplanets, taking into
account survey completeness and, for the first time, \emph{observational
uncertainties}.
We are able to make fewer assumptions about the distribution than previous
studies; we only require that the occurrence rate be a smooth function of
period and radius (employing a Gaussian process).
...
Earth radius.
We find that the rate of Earth analogs is about 0.02 (per star per natural
logarithmic bin in period and radius) with large uncertainty.
...
period and radius of Earth} % Prof Tremaine, you owe us a beer.
\begin{eqnarray}
\gammaearth &=&
