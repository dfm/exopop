...
pgm = daft.PGM([4.5, 2.6], origin=[-0.5, 1])

pgm.add_plate(daft.Plate([0.4, 1.1, 3.1, 2.4], label=r"stars $n=1,\ldots,N$",
                         position="bottom right"))
pgm.add_plate(daft.Plate([0.5, 1.4, 2, 2], label=r"planets $k=1,\ldots,K$"))

pgm.add_node(daft.Node("population", r"\population", 0.0, 3.0, fixed=True))

pgm.add_node(daft.Node("planet", r"$\planet_{kn}$", 1.0, 3.0))
pgm.add_node(daft.Node("planetobs", r"$\planetobs_{kn}$", 2.0, 3.0,
                       observed=True))

pgm.add_node(daft.Node("stellar", r"$\stellar_n$", 3.0, 3.0))
pgm.add_node(daft.Node("stellarobs", r"$\stellarobs_n$", 3.0, 2.0,
                       observed=True))

pgm.add_node(daft.Node("isobs", r"$\isobs_{kn}$", 1.5, 2.0, observed=True))
